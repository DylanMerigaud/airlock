...
def test_incident_url_is_absolute_from_the_relative_overview_url(monkeypatch):
    ...
    # settings.grafana_url() carries the stack's default, so "unset" is an empty setting, not a missing variable
    from airlock import settings

    monkeypatch.setattr(settings, "grafana_url", lambda: "")
...
def test_verdict_sample_no_longer_carries_incidents_total(monkeypatch):
    ...
    # the agent reads the process's shared pushers (airlock.telemetry.shared_pushers), so that is what the test replaces
    monkeypatch.setattr(pipeline, "shared_pushers", lambda: (FakePusher(), None))
    ...
    assert any(ln.startswith("airlock_incident,motive=control_unavailable,owner=platform attached=1i,total=1i") for ln in pushed)
    assert any(ln.startswith("airlock_verdict,motive=control_unavailable,status=BLOCK incidents_total=1i") for ln in pushed)
