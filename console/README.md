...
| Route | What it does |
| --- | --- |
| `POST /api/run` | Resolves the asset to a `gs://` URI, calls Vertex AI Agent Engine `streamQuery`, relays every ADK event to the browser as SSE. Upstream timeout 15 minutes, nothing buffered. An SSE comment every 15 s keeps a proxy's idle timeout from closing the connection during a silent gate (the rights gate can run 30 to 600 s with nothing to relay); the run keeps going server-side regardless (it is Vertex AI Agent Engine's own `streamQuery`, not a process this route owns), but a viewer who still loses the stream has no way back into it yet. |
