...
from airlock.grafana_mcp import DEFAULT_LOKI_UID, DEFAULT_PROM_UID, make_grafana_toolset, pick_datasource_uid, pinned_loki_uid, pinned_prometheus_uid, tool_text
...
def investigator_instruction(ctx: ReadonlyContext) -> str:
    ...
    loki_uid, prom_uid = pinned_loki_uid() or DEFAULT_LOKI_UID, pinned_prometheus_uid() or DEFAULT_PROM_UID
    gate_lines = []
...
class InvestigationBudget:
    ...
    def __init__(self, kind: str, tool_calls: int = INVESTIGATION_TOOL_BUDGET, model_calls: int = INVESTIGATION_MODEL_CALLS, budget_s: float = INVESTIGATION_BUDGET_S) -> None:
        self.kind = kind
...
def make_investigator(budget: InvestigationBudget, toolset: McpToolset) -> LlmAgent:
    ...
    return LlmAgent(
        ...
        generate_content_config=types.GenerateContentConfig(temperature=0.0, max_output_tokens=INVESTIGATION_OUTPUT_TOKENS,
                                                            thinking_config=types.ThinkingConfig(thinking_budget=INVESTIGATION_THINKING_TOKENS, include_thoughts=False)),
            ...
            async with asyncio.timeout(INVESTIGATION_BUDGET_S + 30):
                async for event in investigator.run_async(ctx):
                    for row in tool_rows(event):
                        ...
                        calls.append(row)
                        yield _text_event(ctx, self.name, json.dumps({"stage": "investigation", "run_id": run_id, **row}, default=str), isolation_scope=INVESTIGATION_ROW_SCOPE)
...
class EscalationAgent(BaseAgent):
    ...
    async def _run_async_impl(self, ctx: InvocationContext) -> AsyncGenerator[Event, None]:
        ...
        drill = os.environ.get("AIRLOCK_INCIDENT_DRILL", "true") == "true"
        ...
        try:
            ...
            if existing:
                ...
                if payload.get("incident_id"):
                    ...
                    try:
                        act = tool_text(await tools["add_activity_to_incident"].run_async(args={"incidentId": str(payload["incident_id"]), "body": body}, tool_context=tool_ctx))
                        payload["activity_id"] = json.loads(act).get("activityItemID")
                    except Exception as exc:
            ...
            if not payload.get("incident_id"):
                ...
                ann = tool_text(await tools["create_annotation"].run_async(args={
                    ...
                    "tags": ["airlock", "needs-human", f"owner:{owner}", asset_id[:40], os.environ.get("AIRLOCK_RUNTIME", "local")]}, tool_context=tool_ctx))
                try:
...
def push_incident_sample(status: str, motive: str, owner: str, attached: bool) -> None:
    ...
    InfluxPusher.from_env().push_lines([
...
investigation = InvestigationAgent(name="investigation", description="an LlmAgent on gemini-2.5-flash reads Loki, the counters and the alert rules through mcp-grafana and names the cause")
escalation = EscalationAgent(name="escalation", description="opens or joins a Grafana incident, with the investigator's note, when only a human can arbitrate the BLOCK")
root_agent = SequentialAgent(name="airlock", sub_agents=[gates, verdict, investigation, escalation], description="Airlock: ship or block a generated asset on proof")
